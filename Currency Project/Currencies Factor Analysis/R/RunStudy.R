...
#local_directory <- "~INSEADAnalytics2/CourseSessions/Sessions23"
...
datafile_name="Currencies" # do not add .csv at the end! make sure the data are numeric!!!! check your file!

# Please ENTER the filename of the Report and Slides (in the doc directory) to generate 

report_file = "Currencies_report"
#report_file = "MyBoatsFactor"
slides_file = "Currencies_Slides"

# Please ENTER then original raw attributes to use. 
# Please use numbers, not column names! e.g. c(1:5, 7, 8) uses columns 1,2,3,4,5,7,8
factor_attributes_used= c(2:32)
...
source(paste(local_directory,"R/heatmapO
...
if (start_local_webapp){
  ...
  Currencies <- read.csv(paste(local_directory, "data/Currencies.csv", sep = "/"), sep=";", dec=",") # this contains only the matrix ProjectData

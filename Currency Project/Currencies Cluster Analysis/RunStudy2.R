...
#local_directory <- "~INSEADAnalytics2/CourseSessions/Sessions45"
...
datafile_name="CurrenciesT" # do not add .csv at the end! make sure the data are numeric!!!! check your file!
...
segmentation_attributes_used = c(2:32) 

# Please ENTER then original raw attributes to use for the profiling of the segments (the "profiling attributes")
# Please use numbers, not column names! e.g. c(1:5, 7, 8) uses columns 1,2,3,4,5,7,8
profile_attributes_used = c(2:62) # for boats use c(2:82), for Mall_Visits use c(2:9)
...
if (start_local_webapp){
  ...
  Currencies <- read.csv(paste(local_directory, "data/CurrenciesT.csv", sep = "/"), sep=";", dec=",") # this contains only the matrix ProjectData
